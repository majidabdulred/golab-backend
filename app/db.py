...
async def add_request(request_id,user_id):
    response = await users.update_one({"_id":ObjectId(user_id)},{"$push":{"history":request_id}})
...
async def reset_create_user(email:str,name:str):
    ...
    response = await temp_users.insert_one({"email": email, "name": name,})
...
async def reset_verify_otp(_id: str, otp: int) -> bool:
    ...
    if user.get("otp") == otp and user["expire_after"] > int(time.time()):
        await temp_users.update_one({"_id": ObjectId(str(_id))},{"$set":{"verified":True}})
        return True
    else:
        return False
...
async def reset_new_password(_id: str, password: str) -> None:
    ...
    await users.update_one({"email":user.get("email")},{"$set":{"golab":{"password":auth.get_password_hash(password)}}})
...
async def create_user(name: str, email: str, password: str) -> str:
    ...
    response = await temp_users.insert_one({"name": name, "email": email, "password": auth.get_password_hash(password)})
...
async def create_otp(_id: str) -> None:
    ...
    await temp_users.update_one({"_id": ObjectId(str(_id))},
...
async def get_user_by_email(email: str) -> schema.UserFromDb | None:
    ...
    user = await users.find_one({"email": email})
    if user:
        return schema.UserFromDb(_id=str(user["_id"]),email=user["email"],name=user["name"])
...
async def verify_otp(_id: str, otp: int) -> bool:
    ...
    if not user:
        raise HTTPException(status_code=400, detail="Invalid otp id")
    if user.get("otp") == otp and user["expire_after"] > int(time.time()):
        await users.insert_one({**new_user,"name": user["name"], "email": user["email"], "golab":{"password": user["password"]}})
        await temp_users.delete_one({"_id": ObjectId(str(_id))})
        return True
    else:
        return False
