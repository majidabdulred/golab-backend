...
async def verify_otp(otp_id: str = Body(..., max_length=24, min_length=24), otp: int = Body(...)):
...
async def reset_verify_otp(otp_id: str = Body(..., max_length=24, min_length=24), otp: int = Body(...)):
    """Verify otp and create a user in database"""
...
async def create_txt2img(data: schema.CreateTxt2ImgRequest,
                         user: schema.UserFromDb = Depends(auth.get_current_user)) -> dict:
    """Create a avatar"""
    height, width = util.set_height_and_width(data.aspect_ratio)
    payload = schema.Sendtxt2Img(user_id=user.id, parameters=schema.Parameters(prompt=data.prompt,
                                                                               batch_size=data.num,
                                                                             width=width,
                                                                             height=height,
...
async def create_qr(data: schema.CreateQRRequest,
                    user: schema.UserFromDb = Depends(auth.get_current_user)) -> dict:
    """Create a avatar"""
    payload = schema.SendQR1Request(user_id=user.id,
                                    url=data.url,
                                    prompt=data.preset)
...
async def get_qr_preset():
    ...
    presets = [{"text":i.get("name"),"name":str(i.get("_id")),"image":{"id":i.get("image").split("/")[-1],"url":i.get("image")}}
...
async def get_request_status(id: str,RECURSTION=0):
    ...
    if status == "complete":
        payload = {"status":"COMPLETE",
                   ...
                   "version":1,
                   "request_data":{
                       "prompt":response.get("parameters").get("prompt")
                   },
                   "output":{
                       "images":[{"id":i.split("/")[-1],"url":i} for i in response.get("output").get("images")]}
                   }
        return payload
